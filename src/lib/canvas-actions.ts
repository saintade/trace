import { type Editor, type TLShapeId, sanitizeSvg } from "tldraw";
import {
...
export async function captureBoard(editor: Editor) {
  ...
  // Realtime images travel inside one WebRTC data-channel event. A full-resolution
  // PNG—especially one containing a rendered PDF page—can exceed that channel's
  // per-message limit. Export only what the learner currently sees and progressively
  // reduce a JPEG until the complete data URL is comfortably bounded.
  const attempts = [
    { maxDimension: 1_024, quality: 0.62 },
    { maxDimension: 768, quality: 0.5 },
    { maxDimension: 640, quality: 0.4 },
    { maxDimension: 480, quality: 0.3 },
    { maxDimension: 360, quality: 0.22 },
  ];
  // Leave headroom for the surrounding conversation event on a 64 KiB channel.
  ...
  try {
    for (const attempt of attempts) {
      const scale = Math.min(
        1,
        attempt.maxDimension / Math.max(viewport.w, viewport.h),
      );
      ...
      if (image.url.length <= maxDataUrlLength) return image.url;
    }
    return null;
  } catch {
    return null;
